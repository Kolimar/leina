...
El **scoring** premia los matches más específicos:
...
Desde ahí, la expansión camina en **ambas direcciones** (out + in edges) hasta
...
```mermaid
flowchart LR
    a["LoginHandler"] -->|calls| b["AuthService"]
    b -->|calls| c["TokenFactory"]
    c -->|calls| d["TokenStore"]
```
...
```mermaid
flowchart TD
    start["chequeo de frescura"] --> man{¿hay manifest<br/>válido?}
