...
- **hybrid**: Follow BOTH conventions — persist to Leina memory AND write `tasks.md` to filesystem. Retrieve dependencies from Leina memory (primary) with filesystem fallback.
...
**Change**: {change-name}
**Location**: `openspec/changes/{change-name}/tasks.md` (openspec/hybrid) | Leina memory `sdd/{change-name}/tasks` (memory) | inline (none)
