...
## Rule #0 — Mandatory ticket (NON-NEGOTIABLE)
...
| Rule | Requirement |
|------|-------------|
| **Ticket first** | **Every commit message starts with `TICKET-NNN`. Ask the user for the ticket before committing. Non-negotiable.** |
...
|------------|------------------------|
| `add models` | `PROJ-123 Add token validation domain model and tests` |
| `add services` | `PROJ-123 Wire token validation into login flow` |
