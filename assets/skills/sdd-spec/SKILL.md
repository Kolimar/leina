...
- **hybrid**: Follow BOTH conventions — persist to Leina memory (single concatenated artifact) AND write domain files to filesystem.
...
**IF mode is `memory`:** Existing specs were already retrieved from Leina memory in the Persistence Contract. Skip filesystem reads.
